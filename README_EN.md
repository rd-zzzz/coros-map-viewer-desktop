...
**Roads (`L` layer)**: Each class is drawn as a casing + fill pair — motorways/arterials are peach/salmon, secondary/local roads are white paved at high zoom (turning white around z15) and thin gray lines at low zoom, hiking trails (E29) as well as park paths/pedestrian streets (E13 with `i` 19/23) are black dashed lines, railways are gray dashed lines, and metro/BRT lines are blue; widths are interpolated by zoom and deliberately narrowed so roadside green belts stay visible.
