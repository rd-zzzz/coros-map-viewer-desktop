...
**Roads (`L` layer)**: Each class is drawn as a casing + fill pair — motorways/arterials are peach/salmon with a deeper orange casing; secondary/local roads are white paved at high zoom (secondary around z14.5, local around z15) and thin gray lines at low zoom, and the white pavement is outlined with medium-gray casings (darker for secondary than local) so white roads do not vanish against the near-white background. Hiking trails (E29) and park paths/pedestrian streets (E13 with `i` 19/23) are medium-gray dashed lines, and paths (E0) are thin light-gray lines; both colors and widths are deliberately weaker than formal roads so low-class paths never outshine major roads. Railways are gray dashed lines, and metro/BRT lines are blue; widths are interpolated by zoom and deliberately narrowed so roadside green belts stay visible. Hiking trails are further faded below z13 (opacity 0.55) and fully opaque from z15, reducing dashed-line noise at low zoom.

**Land cover and water (`F` / `N` layers)**: Forests are light green, farmland pale yellow, urban parks teal, water light blue; protected areas and airport land have their own base colors. Land cover fades in between z9 and z11.5 so color patches do not dominate low zoom levels.

**POIs (`K` layer)**: Uniform blue round icons with white glyphs (sprite under `src/sprites/coros/`), generated by the build script `tools/build-coros-sprite.js`, which extracts white-glyph masks from the v4/light sprite; missing icons (hospital, parking, fuel, campsite, golf) are hand-drawn by the script. POIs are tiered by importance — major categories show icons from z12.5 and names from z13.5, while minor ones are deferred to z14.5 / z15, keeping dense urban areas uncluttered.

**Text labels**: Names are read from the COROS `X` field (the standard `name` is absent). Road names follow the road line in dark gray; administrative names are dark red with Medium weight and sizes graded by administrative level (province 14 down to village 10); POI names are neutral gray and peak names brown — all with a white halo.

**VCM (contours, `Q` layer)**: Index/contour lines are distinguished by `F` (elevation) `% 50`, drawn as thin tan lines with fixed opacity (0.75 for intermediate, 0.9 for index); index lines use a slightly deeper color, thicken with zoom (0.75 at z9 to 1.5 at z13), and carry elevation labels along the line from z10.5. There is no hover highlight: contour elevation is inherently non-unique (in a sample of 36 z11 tiles, 43 features carried only 5 distinct elevations), so it cannot serve as a feature id — `feature-state` would light up every same-elevation line across the whole source.
...
| Control panel | Top-left | Folder selection (dashed drag area, supports native drag-drop), VCM toggle, light/dark theme toggle, statistics cards (indexed / active), progress bar, cancel-index button, clear-all/file-list buttons, failed-file details and an about section; panel is collapsible |
| File list | Drawer right of the control panel | All indexed files grouped by VCM/VSM/PMT with zoom ranges; click a file to fly to its coverage; header shows per-type counts |
| Empty state | Map center | Shows guidance when no file is loaded; a full-screen drag overlay appears while files are dragged in from Explorer |
...
| Status bar | Bottom 28px | Mouse coordinates and loading status indicator; the title bar shows the current folder name after indexing |
...
Window position, size and maximized state are automatically remembered and restored on the Rust side by `tauri-plugin-window-state` (no frontend permissions required).
...
| `src-tauri/target/release/bundle/nsis/MapViewer_0.3.1_x64-setup.exe` | ~6.8 MB | NSIS installer |
...
| tauri-plugin-window-state | 2.x | Remembers/restores window position and size | MIT/Apache-2.0 |
