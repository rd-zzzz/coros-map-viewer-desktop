...
**Text labels**: Names are read from the COROS `X` field (the standard `name` is absent). Road names follow the road line in dark gray, administrative names are dark red, POI names neutral gray, peak names brown, all with a white halo.
...
| Control panel | Top-left | Folder selection (dashed drag area, supports native drag-drop), VCM toggle, light/dark theme toggle, statistics cards (indexed / active), progress bar, clear all button; panel is collapsible |
