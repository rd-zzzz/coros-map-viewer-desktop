...
**道路（`L` 层）**：每个等级绘制 casing（描边）+ fill（铺面）两层——高速/主干道为 peach/salmon 色，次干/支路在高缩放为白色铺面（约 z15 变白）、低缩放为浅灰细线，登山步道（E29）以及公园步道/步行街（E13 且 `i` 为 19/23）为黑色虚线，铁路为灰色虚线，地铁/BRT 为蓝色；线宽随缩放插值，并刻意收窄以让沿路绿带露出。
...
**文字标注**：名称读取 COROS 的 `X` 字段（标准 `name` 缺失）；道路名沿道路走向排布、深灰色，行政地名与 POI 名暗红色、山峰名棕褐色，均带白色描边光晕。

**VCM（等高线，`Q` 层）**：按 `F`（高程）`% 50` 区分首曲线/计曲线，淡棕褐色细线；hover 时不透明度提升（依赖 `promoteId` 从高程字段 `F` 提升要素 id）。

**全局叠放顺序**：所有图层按 `SLOT_ORDER`（地表/水体 → 道路由支路到高速 → 步道/铁路 → 图标 → 各类文字，自底向顶）在每次 `loadEntry()` 后由 `reorderLayers()` 通过 `moveLayer` 重排，保证多源叠放一致。
