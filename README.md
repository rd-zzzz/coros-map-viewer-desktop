...
| 图层 | 几何 | 语义（`E` 分类码） |
|---|---|---|
| `L` | 线/面 | 道路：7/20 高速、8/9 主干、10 次干、11–14/23 支路、0 小路、29 登山步道、2 铁路、3 地铁/BRT |
...
**道路（`L` 层）**：每个等级绘制 casing（描边）+ fill（铺面）两层——高速/主干道为 peach/salmon 色，次干/支路在高缩放为白色铺面、低缩放为浅灰细线，登山步道（E29）为黑色虚线，铁路为灰色虚线，地铁/BRT 为蓝色；线宽随缩放插值。
...
**文字标注**：道路名深灰色、行政地名与 POI 名暗红色、山峰名棕褐色，均带白色描边光晕。
