...
**道路（`L` 层）**：每个等级绘制 casing（描边）+ fill（铺面）两层——高速/主干道为 peach/salmon 色并配更深的橙色描边；次干/支路在高缩放为白色铺面（次干约 z14.5、支路约 z15 变白）、低缩放为浅灰细线，白色铺面统一用中灰色 casing 勾边（次干描边深于支路），避免白色道路在近白背景上「消失」。登山步道（E29）以及公园步道/步行街（E13 且 `i` 为 19/23）为中灰色虚线、小路（E0）为浅灰细线，颜色与线宽均刻意弱于正式道路，防止低等级小路比大路更抢眼；铁路为灰色虚线，地铁/BRT 为蓝色。线宽随缩放插值，并刻意收窄以让沿路绿带露出。登山步道在 z13 以下进一步淡化（不透明度 0.55）、z15 起完全不透明，减少低缩放时的虚线噪声。

**地表与水体（`F` / `N` 层）**：林地淡绿、农田淡黄、城市公园 teal 色、水体淡蓝；保护区与机场用地各有对应底色。地表覆盖在 z9–z11.5 间淡入，避免低缩放时色斑过重。

**POI（`K` 层）**：使用统一的蓝色圆形 + 白色字形图标（精灵位于 `src/sprites/coros/`），由构建脚本 `tools/build-coros-sprite.js` 从 v4/light 精灵提取白色字形掩膜生成；缺失图标（医院、停车、加油、露营、高尔夫）由脚本手绘。POI 按重要度分级——重要类别 z12.5 起显示图标、z13.5 起显示名称；次要类别分别推迟到 z14.5 / z15，避免城区图标与文字过密。

**文字标注**：名称读取 COROS 的 `X` 字段（标准 `name` 缺失）；道路名沿道路走向排布、深灰色，行政地名暗红色并使用 Medium 字重、字号按行政层级分档（省级 14 → 村级 10），POI 名中性灰、山峰名棕褐色，均带白色描边光晕。

**VCM（等高线，`Q` 层）**：按 `F`（高程）`% 50` 区分首曲线/计曲线，淡棕褐色细线，不透明度固定（首曲线 0.75、计曲线 0.9）；计曲线颜色略深、线宽随缩放加粗（z9 0.75 → z13 1.5），并自 z10.5 起沿计曲线走向标注高程数字。不提供 hover 高亮：等高线的高程天然不唯一（实测 36 个 z11 瓦片里 43 个要素只有 5 个高程值），无法作为要素 id，用 `feature-state` 会跨瓦片点亮所有同高程线。
...
| 控件面板 | 左上角 | 文件夹选择（虚线拖拽区，支持原生拖拽）、VCM 开关、浅色/深色主题切换、统计卡片（已索引 / 已加载）、进度条、取消索引按钮、清除所有/文件列表按钮、失败文件明细、关于区；面板可折叠 |
| 文件列表 | 控件面板右侧抽屉 | 按 VCM/VSM/PMT 分类列出全部已索引文件及缩放范围，点击任意文件定位到其覆盖范围，头部汇总各类数量 |
| 空状态 | 地图中央 | 未加载任何文件时显示操作引导；从资源管理器拖入文件时全屏显示拖拽遮罩 |
...
| 状态栏 | 底部 28px | 鼠标坐标、加载状态指示灯；标题栏在索引后显示当前文件夹名 |
...
窗口位置、尺寸与最大化状态由 `tauri-plugin-window-state` 在 Rust 侧自动记忆与恢复（无需前端权限）。
...
| tauri-plugin-window-state | 2.x | 记忆/恢复窗口位置与尺寸 | MIT/Apache-2.0 |
